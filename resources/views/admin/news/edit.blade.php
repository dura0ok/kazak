...
@section("content")
    ...
    <form action="{{ route('news.update', $article->id) }}" method="POST" class="creation_form" enctype="multipart/form-data">
        @csrf
        {{ method_field('PUT') }}
        @if($errors->any())
        ...
        <label for="additionalImages">Вставьте дополнительные в поле ниже(выберите из своей файловой системы)(старые  сотрутся, а то что вы прикрепите будут)</label>
